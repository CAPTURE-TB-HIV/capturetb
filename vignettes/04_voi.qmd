...
When using model estimates in the context of an HTA, model uncertainty will have a certain cost associated with it. The \link{unitcost} model has a built-in method for calulating the Expected Value of Perfect Information (EVPI) - in other words, the cost of model uncertainty - given a willingness-to-pay threshold for a single treatment visit.
