...
# suppose over-prediction is twice as bad
alpha <- 2
beta <- 1
...
loss_data <- data.frame(loss_function = "Asymmetric linear",
		mean_loss,
	 	lower_loss, 
	 	upper_loss
	 )
...
loss_data <- rbind(loss_data, 
	data.frame(loss_function = "Symmetric linear", 
		mean_loss,
	 	lower_loss,
	  	upper_loss))

rownames(loss_data) <- NULL
