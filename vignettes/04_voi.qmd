...
When using model estimates in the context of an HTA, model uncertainty will have a certain cost associated with it. The models in this package have a built-in method for calulating the Expected Value of Perfect Information (EVPI) - in other words, the cost of model uncertainty - given a willingness-to-pay threshold.
...
prepared_inputs <- prepare_covariates(new_inputs, model)
pred <- model$predict(prepared_inputs, scale = "natural", summarised = TRUE)
...
```{r, plot-evpi}
lambda <- c(3:12)
evpi <- model$evpi(prepared_inputs, lambda)
...
As well as calculating EVPI for costs at a single facility, the method can be used to look at EVPI for an intervention that involve $n_i$ visits for facilities $i = 1 ...I$

```{r, total-evpi}
# using model training data to demonstrate
# but works for any inputs
dat <- dat |> filter(output == "op_treatmentvisit")
n_output <- dat$fc_opvisits_TB
lambda <- seq(5000000, 7500000, by = 10000)
evpi <- model$evpi(dat, lambda, n_output)
ggplot(data.frame(lambda, evpi)) + 
	geom_line(aes(lambda, evpi)) +
...
Here we demonstrate how to calculate (and minimise) expected loss given an asymmetric linear loss function:
...
Bayesian decision theory shows that the optimal budget under an asymmetric linear function is the $\dfrac{\alpha}{\alpha + \beta}$ quantile of the posterior cost distribution, call it $c*$. The expected loss associated with using this prediction is then 
...
loss <- sapply(samples, loss_function, c_star = c_star)

# loss if we used the mean prediction instead
loss2 <- sapply(samples, loss_function, c_star = c_mean)
...
In some cases there may not be an analytical solution for minimising losses. In this case the `optimize` function can be used.

```{r}
# define the expected loss function
expected_loss <- function(c_star) {
	mean(sapply(samples, loss_function, c_star = c_star))
}

# find the minimum
res <- optimize(expected_loss, c(0, max(samples)))

# in this case, the result should agree with the analytic solution
knitr::kable(data.frame(
	numerical_optimisation = scales::dollar(res$minimum),
  analytic=scales::dollar(unname(c_star))
))
```
